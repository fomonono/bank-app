...
searchAcc.addEventListener("submit", function (event) {
  ...
  axios
    .get(`http://localhost:5050/accounts/number/${inputValue}`)
    .then((response) => {
...
function renderUser(
  ...
  TransactionBtn.setAttribute(
    "class",
    "bg-[#2e3a4d] rounded text-white py-1 px-3 buttonClick"
  );
  ...
  UpdateBtn.setAttribute(
    "class",
    "bg-[#00b94b] rounded text-white py-1 px-3 buttonClick"
  );
  ...
  DeleteBtn.setAttribute(
    "class",
    "bg-[#d5263e] rounded text-white py-1 px-3 buttonClick"
  );
...
function createAccount() {
  ...
  axios
    .post("http://localhost:5050/accounts", body)
    .then((response) => {
...
function transferMoneyFunc() {
  const formData = new FormData(transferMoney);
  const body = Object.fromEntries(formData);

  axios
    .post("http://localhost:5050/transfer", {
    ...
    .then((response) => {
      ...
      transferMoney.appendChild(transferMessage);
      transferMessage.classList.remove("hidden");
      transferMessage.classList.add(
        "text-green-600",
        "text-center",
        "text-lg",
        "font-semibold"
      );
      transferMessage.innerText = response.data.message;
      transferMoney.reset();
...
transferMoney.addEventListener("submit", (event) => {
  event.preventDefault();
  transferMoneyFunc();
});

function withdrawMoneyFunc() {
  const formData = new FormData(withdrawMoney);
  const body = Object.fromEntries(formData);
  ...
  axios
    ...
    .then((response) => {
      ...
      withdrawMessage.classList.add("text-green-600");
      withdrawMessage.innerText = response.data.message;
      withdrawMoney.reset();
    ...
    .catch((error) => {
      ...
      withdrawMessage.innerText =
        error.response?.data?.error || "Failed to withdraw money.";
      alert("Failed to withdraw money.");
    });
}

withdrawMoney.addEventListener("submit", (event) => {
  event.preventDefault();
  withdrawMoneyFunc();
});
